...
pdf(file.path('graphs', 'log_lambda_rmse.pdf'))
print(ggplot(performance, aes(x = Lambda, y = RMSE)) +
  stat_summary(fun.data = 'mean_cl_boot', geom = 'errorbar') +
  scale_x_log10() +
  xlab('Lambda') +
  ylab('Cross-Validated RMSE') +
  opts(title = 'Lasso Hyperparameter Tuning'))
dev.off()
...
pdf(file.path('graphs', 'log_ideal_point_predictions.pdf'))
print(ggplot(predictions, aes(x = Predicted, y = Empirical)) +
  ...
  opts(title = 'True vs. Predicted Ideal Points'))
dev.off()
pdf(file.path('graphs', 'log_ideal_point_residuals.pdf'))
print(ggplot(predictions, aes(x = Predicted, y = Residual)) +
  geom_point() +
  xlab('Document\'s Predicted Ideal Point') +
  ylab('Document\'s Residual') +
  opts(title = 'Predicted Ideal Points vs. Residuals'))
dev.off()
...
pdf(file.path('graphs', 'log_test_set_performance.pdf'))
...
write.csv(data.frame(Term = names(most.democratic.terms),
          ...
          file.path('cache', 'most_democratic_terms_log.csv'),
          row.names = FALSE)
...
write.csv(data.frame(Term = names(most.republican.terms),
          ...
          file.path('cache', 'most_republican_terms_log.csv'),
          row.names = FALSE)

# Histogram of final coefficients.
theta <- term.weights[,1]
parameters <- data.frame(Theta = as.numeric(theta), Party = ifelse(theta < 0, 'Democratic', 'Republican'))
pdf(file.path('graphs', 'log_estimated_coefficients.pdf'))
print(ggplot(subset(parameters, Theta != 0), aes(x = Theta, fill = Party)) +
  geom_density() +
  xlim(c(-1.3, 1.3)) +
  xlab('Coefficient Size') +
  ylab('Estimated Density') +
  opts(title = 'Distribution of Estimated Non-Zero Coefficients'))
dev.off()
